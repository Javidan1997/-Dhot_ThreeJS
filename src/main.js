...
document.addEventListener('DOMContentLoaded', () => {
   ...
   // Raycasting for selecting parts
    ...
    function getMaxMovementValues(selectedSize, selectedGlasses) {
        let maxMovementX, maxMovementY, maxMovementY_1;
        if (selectedSize === "14'x20'") {
            if (selectedGlasses.front || selectedGlasses.rear) {
                maxMovementX = 75;
                maxMovementY = 35;
                maxMovementY_1 = 35;
            } else if (selectedGlasses.left || selectedGlasses.right) {
                maxMovementX = 115;
                maxMovementY = -35;
                maxMovementY_1 = -70;
            }
    ...
    glassControl.addEventListener('input', (event) => {
        ...
        // Get movement values based on selected size and glasses
        const { maxMovementX, maxMovementY, maxMovementY_1 } = getMaxMovementValues(selectedSize, selectedGlasses);

        // Reset all parts to their initial positions before translation
        glassAnimationParts.forEach((part) => {
            part.position.set(
        ...
        const normalParts = glassAnimationParts.filter(part =>
            part.name.includes('Sliding_Normal') && !part.name.includes('Sliding_Normal_1')
        );
        const normalParts_1 = glassAnimationParts.filter(part =>
            part.name.includes('Sliding_Normal_1')
        );
        const door2Parts = glassAnimationParts.filter(part =>
            part.name.includes('Sliding_Lock_Door_2')
        );

        // Function to slide Normal parts towards a target Y position
        function slideNormalParts(parts, targetPositionY) {
            parts.forEach((part) => {
                const translateDistanceY = slideAmount * targetPositionY;
                part.position.y = part.userData.initialY - translateDistanceY;
            });
        }

        // Function to slide Door_2 parts towards a target X position
        function slideDoor2Parts(parts, targetPositionX) {
            parts.forEach((part) => {
                // Calculate the current movement based on slideAmount
                const translateDistanceX = slideAmount * targetPositionX;
                part.position.x = part.userData.initialX - translateDistanceX;
            });
        }

        // Slide Normal Parts (Sliding_Normal) towards the reference part along Y-axis
        slideNormalParts(normalParts, maxMovementY);
        slideNormalParts(normalParts_1, maxMovementY_1);

        // Slide Door_2 Parts (Sliding_Lock_Door_2) behind the Normal parts along X-axis
        slideDoor2Parts(door2Parts, maxMovementX);
    });
    function animateGlassSliding(side) {
        ...
        normalParts.forEach(part => {
            part.position.y = part.userData.initialY;
        });
        normalParts_1.forEach(part => {
            part.position.y = part.userData.initialY;
        });
        door2Parts.forEach(part => {
            part.position.x = part.userData.initialX;
        });
    
        // Animate sliding in
        animateParts(normalParts, 'y', part => part.userData.initialY - maxMovementY, duration);
        animateParts(normalParts_1, 'y', part => part.userData.initialY - maxMovementY_1, duration);
        animateParts(door2Parts, 'x', part => part.userData.initialX - maxMovementX, duration, () => {
            // After sliding in, slide back
            animateParts(normalParts, 'y', part => part.userData.initialY, duration);
            animateParts(normalParts_1, 'y', part => part.userData.initialY, duration);
            animateParts(door2Parts, 'x', part => part.userData.initialX, duration);
        });
    }
    
    function animateParts(parts, axis, targetPositionFunc, duration, onComplete) {
        const startTime = performance.now();
        const initialPositions = parts.map(part => part.position[axis]);
        ...
        function animate(time) {
            ...
            parts.forEach((part, index) => {
                ...
                const targetPosition = targetPositionFunc(part);
                part.position[axis] = initialPosition + (targetPosition - initialPosition) * progress;
            });
